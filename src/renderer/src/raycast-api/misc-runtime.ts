...
export function createDeeplink(
  ...
  if (options.type === DeeplinkType.ScriptCommand) {
    ...
    return `discov://script-commands/${encodeURIComponent(options.command)}${qs ? `?${qs}` : ''}`;
  }
  ...
  const qs = params.toString();
  return `discov://extensions/${encodeURIComponent(owner)}/${encodeURIComponent(extName)}/${encodeURIComponent(options.command)}${qs ? `?${qs}` : ''}`;
}
